...
int main(void) { 
  ...
  char process [100];  // array that will evaluate the expression
  ...
  int e = 0; // used as index in expression to delete any whitespaces	
  int f = 0;
  printf("Please enter a mathematical expression \n");
  fflush(stdout);	
  scanf(" %[^\n]s ",process);              //allows for whitespace in input
  while (e !=  strlen(process)) {          //algorithm that gets rid of unecessary
	  if (isspace(process[f]) == 0) {      //whitespaces by removing them as string
		  expression[e] = process[f];      //gets copied into expression
		  //puts("executed");	
		  e++;
	  }
	  
	  //printf("the size of the string now : %d\n, and the index in question %d\n",strlen(process),f);
	  f++;
  } 
	printf(" your expression is %s",process);
	return 0;
